...
struct LuaStack
{
    ...
    int num_args() { return lua_gettop(L); }
    ...
    bool is_nil(int i) { return lua_isnil(L, i) == 1; }

    bool is_bool(int i) { return lua_isboolean(L, i) == 1; }

    bool is_number(int i) { return lua_isnumber(L, i) == 1; }

    bool is_string(int i) { return lua_isstring(L, i) == 1; }
    ...
    bool is_function(int i) { return lua_isfunction(L, i) == 1; }

    bool is_table(int i) { return lua_istable(L, i) == 1; }
    ...
    /// Wraps lua_type.
    int value_type(int i) { return lua_type(L, i); }

    bool get_bool(int i) { return lua_toboolean(L, i) == 1; }
    ...
    StringId32 get_string_id_32(int i) { return StringId32(get_string(i)); }

    StringId64 get_string_id_64(int i) { return StringId64(get_string(i)); }

    StringId64 get_resource_id(int i) { return StringId64(get_string(i)); }
    ...
    Material *get_material(int i) { return (Material *)get_pointer(i); }
    ...
    SoundInstanceId get_sound_instance_id(int i) { return get_id(i); }
    ...
    void push_nil() { lua_pushnil(L); }

    void push_bool(bool value) { lua_pushboolean(L, value); }

    void push_int(int value) { lua_pushnumber(L, value); }

    void push_float(f32 value) { lua_pushnumber(L, value); }

    void push_string(const char *s) { lua_pushstring(L, s); }
    ...
    void push_lstring(const char *s, u32 len) { lua_pushlstring(L, s, len); }

    void push_string_id(StringId32 value) { lua_pushnumber(L, value._id); }
    ...
    void push_function(lua_CFunction f) { lua_pushcfunction(L, f); }

    void push_id(u32 value) { lua_pushnumber(L, value); }

    /// Pushes an empty table onto the stack.
    /// When you want to set keys on the table, you have to use
    /// LuaStack::push_key_begin() and LuaStack::push_key_end() as in the
    /// following example:
    ///
    /// LuaStack stack(L)
    /// stack.push_table()
    ...
    void push_key_begin(const char *key) { lua_pushstring(L, key); }

    /// See Stack::push_table()
    void push_key_begin(int i) { lua_pushnumber(L, i); }

    /// See Stack::push_table()
    void push_key_end() { lua_settable(L, -3); }

    int next(int i) { return lua_next(L, i); }

    void push_gui(Gui *dg) { push_pointer(dg); }

    void push_debug_line(DebugLine *line) { push_pointer(line); }
    ...
    void push_world(World *world) { push_pointer(world); };

    void push_scene_graph(SceneGraph *sg) { push_pointer(sg); }

    void push_level(Level *level) { push_pointer(level); }

    void push_render_world(RenderWorld *world) { push_pointer(world); }

    void push_physics_world(PhysicsWorld *world) { push_pointer(world); }

    void push_sound_world(SoundWorld *world) { push_pointer(world); }

    void push_script_world(ScriptWorld *world) { push_pointer(world); }
    ...
    void push_camera(CameraInstance i) { push_id(i.i); }

    void push_transform(TransformInstance i) { push_id(i.i); }

    void push_mesh_instance(MeshInstance i) { push_id(i.i); }

    void push_sprite_instance(SpriteInstance i) { push_id(i.i); }

    void push_light_instance(LightInstance i) { push_id(i.i); }

    void push_material(Material *material) { push_pointer(material); }

    void push_actor(ActorInstance i) { push_id(i.i); }

    void push_sound_instance_id(SoundInstanceId id) { push_id(id); }

    void push_script_instance(ScriptInstance i) { push_id(i.i); }
    ...
    void call(int nresults) { lua_pcall(L, 2, nresults, 0); }
    ...
    {
        if (!is_pointer(i) || *(u32 *)p != RESOURZE_PACKAGE_MARKER)
        {
