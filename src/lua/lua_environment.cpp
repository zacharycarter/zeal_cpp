...
LuaEnvironment::LuaEnvironment()
    : L(NULL), _num_vec3(0), _num_quat(0), _num_mat4(0)
{
    L = luaL_newstate();
    ZE_ASSERT(L, "Unable to create lua state");
}

LuaEnvironment::~LuaEnvironment() { lua_close(L); }
...
void LuaEnvironment::load_libs()
{
    ...
    ZE_ASSERT(lua_gettop(L) == 0, "Stack not clean");
...
void LuaEnvironment::add_module_function(const char *module, const char *name,
...
void LuaEnvironment::add_module_metafunction(const char *module,
                                             const char *name,
                                             const lua_CFunction func)
{
    // Create module if it does not exist
...
void LuaEnvironment::call_global(const char *func, u8 argc, ...)
{
    ...
    for (u8 i = 0; i < argc; i++)
    {
        ...
        default:
            ZE_FATAL("Unknown argument type");
            break;
        }
    }
