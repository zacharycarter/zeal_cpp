...
SOCKET open()
{
    ...
    ZE_ASSERT(socket >= 0, "socket: last_error() = %d", last_error());
    return socket;
}
...
ReadResult read(SOCKET socket, void *data, u32 size)
{
    ...
    while (to_read > 0)
    {
        int bytes_read = ::recv(socket, (char *)data + rr.bytes_read, to_read, 0);
...
WriteResult write(SOCKET socket, const void *data, u32 size)
{
    ...
    {
        int bytes_wrote =
            ::send(socket, (char *)data + wr.bytes_wrote, to_write, 0);
...
void set_blocking(SOCKET socket, bool blocking)
{
    ...
    ZE_ASSERT(err == 0, "ioctlsocket: last_error() = %d", last_error());
    ZE_UNUSED(err);
#endif
}
...
void set_reuse_address(SOCKET socket, bool reuse)
{
    ...
    int err = setsockopt(socket, SOL_SOCKET, SO_REUSEADDR, (const char *)&optval,
                         sizeof(optval));
    ZE_ASSERT(err == 0, "setsockopt: last_error() = %d", last_error());
    ZE_UNUSED(err);
}
...
void set_timeout(SOCKET socket, u32 ms)
{
    ...
    int err = setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv,
                         sizeof(tv));
    ZE_ASSERT(err == 0, "setsockopt: last_error(): %d", last_error());
    err = setsockopt(socket, SOL_SOCKET, SO_SNDTIMEO, (const char *)&tv,
                     sizeof(tv));
    ZE_ASSERT(err == 0, "setsockopt: last_error(): %d", last_error());
    ZE_UNUSED(err);
}
...
TCPSocket::TCPSocket()
{
    ZE_STATIC_ASSERT(sizeof(_data) >= sizeof(SOCKET));
...
ConnectResult TCPSocket::connect(const IPAddress &ip, u16 port)
{
    ...
    int err =
        ::connect(priv->socket, (const sockaddr *)&addr_in, sizeof(sockaddr_in));
...
BindResult TCPSocket::bind(u16 port)
{
    ...
    int err =
        ::bind(priv->socket, (const sockaddr *)&address, sizeof(sockaddr_in));
...
void TCPSocket::listen(u32 max)
{
    ...
    ZE_ASSERT(err == 0, "listen: last_error() = %d", last_error());
    ZE_UNUSED(err);
}
