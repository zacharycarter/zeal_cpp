...
struct HeapAllocator : public Allocator
{
	...
	HeapAllocator() : _allocated_size(0), _allocation_count(0) {}

	~HeapAllocator()
	{
		ZE_ASSERT(_allocation_count == 0 && total_allocated() == 0,
				  "Missing %d deallocations causing a leak of %d bytes",
				  _allocation_count, total_allocated());
	}
	...
	u32 allocated_size(const void *ptr) { return get_size(ptr); }
...
struct ScratchAllocator : public Allocator
{
	...
	~ScratchAllocator()
	{
		ZE_ASSERT(_free == _allocate, "Memory leak");
	...
	void *allocate(u32 size, u32 align)
	{
		...
		ZE_ASSERT(align % 4 == 0, "Must be 4-byte aligned");
		size = ((size + 3) / 4) * 4;
	...
	void deallocate(void *p)
	{
		...
		Header *h = header(p);
		ZE_ASSERT((h->size & 0x80000000u) == 0, "Not free");
...
void init()
{
	...
	_default_scratch_allocator = new (_buffer + sizeof(HeapAllocator))
		ScratchAllocator(*_default_allocator, 1024 * 1024);
}
...
Allocator &default_allocator() { return *memory_globals::_default_allocator; }
