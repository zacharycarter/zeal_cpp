...
static const char *next(const char *json, const char c = 0)
{
    ...
    if (c && c != *json)
    {
        ZE_ASSERT(false, "Expected '%c' got '%c'", c, *json);
    }
...
static const char *skip_value(const char *json)
{
    ...
    case '[':
        json = skip_block(json, '[', ']');
        break;
    case '{':
        json = skip_block(json, '{', '}');
        break;
    default:
        for (; *json != '\0' && *json != ',' && *json != '\n' && *json != ' ' &&
               *json != '}' && *json != ']';
             ++json)
            ;
        break;
...
static f64 parse_number(const char *json)
{
    ...
    ZE_ASSERT(ok == 1, "Failed to parse f64: %s", array::begin(number));
    ZE_UNUSED(ok);
    return val;
}

s32 parse_int(const char *json) { return (s32)parse_number(json); }

f32 parse_float(const char *json) { return (f32)parse_number(json); }
...
void parse_verbatim(const char *json, DynamicString &string)
{
    ...
    ZE_ASSERT(end, "Bad verbatim string");
