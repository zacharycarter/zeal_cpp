#ifndef ZEAL_QUATERNION_H_HEADER_GUARD
#define ZEAL_QUATERNION_H_HEADER_GUARD
...
/// Multiplies the quaternions @a a by @a b and returns the result. (i.e.
/// rotates first by @a a then by @a b).
...
inline f32 length(const Quaternion &q) { return fsqrt(dot(q, q)); }
...
/// Returns the quaternion describing the rotation needed to face towards @a
/// dir.
...
/// Returns the linearly interpolated quaternion between *a* and *b* at time *t*
/// in [0, 1]. It uses NLerp.
...
#endif // ZEAL_QUATERNION_H_HEADER_GUARD