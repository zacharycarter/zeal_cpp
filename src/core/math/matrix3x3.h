#ifndef ZEAL_MATRIX3X3_H_HEADER_GUARD
#define ZEAL_MATRIX3X3_H_HEADER_GUARD
...
inline Matrix3x3 matrix3x3(f32 xx, f32 xy, f32 xz, f32 yx, f32 yy, f32 yz,
...
inline Matrix3x3 matrix3x3(const Vector3 &x, const Vector3 &y,
                           const Vector3 &z)
{
    Matrix3x3 m;
    m.x = x;
...
/// Multiplies the matrix @a a by @a b and returns the result. (i.e. transforms
/// first by @a a then by @a b)
inline Matrix3x3 &operator*=(Matrix3x3 &a, const Matrix3x3 &b)
{
    Matrix3x3 tmp;
...
inline f32 *to_float_ptr(Matrix3x3 &m) { return &m.x.x; }

/// Returns the pointer to the matrix's data
inline const f32 *to_float_ptr(const Matrix3x3 &m) { return &m.x.x; }

/// @}

} // namespace zeal

#endif // ZEAL_MATRIX3X3_H_HEADER_GUARD