#ifndef ZEAL_QUEUE_H_HEADER_GUARD
#define ZEAL_QUEUE_H_HEADER_GUARD
...
template <typename T>
void increase_capacity(Queue<T> &q, u32 capacity);
...
/// time of call.
template <typename T>
void grow(Queue<T> &q, u32 min_capacity);

/// Appends an @a item to the back of the queue
template <typename T>
void push_back(Queue<T> &q, const T &item);
...
template <typename T>
void push_front(Queue<T> &q, const T &item);
...
template <typename T>
void push(Queue<T> &q, const T *items, u32 n);

/// Removes @a n items from the front of the queue
template <typename T>
void pop(Queue<T> &q, u32 n);
...
template <typename T>
inline u32 size(const Queue<T> &q) { return q._size; }
...
inline void increase_capacity(Queue<T> &q, u32 capacity)
{
    ...
    {
        memmove(array::begin(q._queue) + capacity - (old_size - q._read),
                array::begin(q._queue) + q._read, (old_size - q._read) * sizeof(T));
...
{
    ZE_ASSERT(q._size > 0, "The queue is empty");
    --q._size;
}
...
inline T *end(Queue<T> &q)
{
    ...
    return end >= array::size(q._queue) ? array::end(q._queue)
                                        : array::begin(q._queue) + end;
}
...
template <typename T>
inline Queue<T>::Queue(Allocator &a) : _read(0), _size(0), _queue(a) {}
...
#endif // ZEAL_QUEUE_H_HEADER_GUARD