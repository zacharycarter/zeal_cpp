#ifndef ZEAL_HASH_MAP_H_HEADER_GUARD
#define ZEAL_HASH_MAP_H_HEADER_GUARD
...
template <typename TKey, typename TValue, typename Hash, typename KeyEqual>
const TValue &get(const HashMap<TKey, TValue, Hash, KeyEqual> &m,
                  const TKey &key, const TValue &deffault);

/// Sets the @a value for the @a key in the map.
template <typename TKey, typename TValue, typename Hash, typename KeyEqual>
void set(HashMap<TKey, TValue, Hash, KeyEqual> &m, const TKey &key,
         const TValue &value);
...
template <typename TKey, typename Hash> inline u32 key_hash(const TKey &key) {
  const Hash hash;
  return hash(key);
}
...
template <typename TKey, typename TValue, typename Hash, typename KeyEqual>
inline u32 probe_distance(const HashMap<TKey, TValue, Hash, KeyEqual> &m,
                          u32 hash, u32 slot_index) {
...
u32 find(const HashMap<TKey, TValue, Hash, KeyEqual> &m, const TKey &key) {
  ...
  for (;;) {
    if (m._index[hash_i].index == FREE)
      ...
      return END_OF_LIST;
    else if (!is_deleted(m._index[hash_i].index) &&
             m._index[hash_i].hash == hash &&
             key_equals<TKey, KeyEqual>(m._data[hash_i].first, key))
      return hash_i;
...
template <typename TKey, typename TValue, typename Hash, typename KeyEqual>
void insert(HashMap<TKey, TValue, Hash, KeyEqual> &m, u32 hash, const TKey &key,
            const TValue &value) {
  PAIR(TKey, TValue)
  new_item(*m._allocator);
  new_item.first = key;
  ...
  for (;;) {
    ...
    u32 existing_elem_probe_dist =
        probe_distance(m, m._index[hash_i].hash, hash_i);
...
void rehash(HashMap<TKey, TValue, Hash, KeyEqual> &m, u32 new_capacity) {
  ...
  const u32 size = new_capacity * (sizeof(Index) + sizeof(Entry)) +
                   alignof(Index) + alignof(Entry);
  nm._buffer = (char *)nm._allocator->allocate(size);
  nm._index = (Index *)memory::align_top(nm._buffer, alignof(Index));
  nm._data =
      (Entry *)memory::align_top(nm._index + new_capacity, alignof(Entry));
  ...
  memcpy((void *)&m, (void *)&nm,
         sizeof(HashMap<TKey, TValue, Hash, KeyEqual>));
  memcpy((void *)&nm, (void *)&empty,
         sizeof(HashMap<TKey, TValue, Hash, KeyEqual>));
}
...
void set(HashMap<TKey, TValue, Hash, KeyEqual> &m, const TKey &key,
  ...
  const u32 i = hash_map_internal::find(m, key);
  if (i == hash_map_internal::END_OF_LIST) {
    hash_map_internal::insert(m, hash_map_internal::key_hash<TKey, Hash>(key),
                              key, value);
    ++m._size;
  } else {
...
HashMap<TKey, TValue, Hash, KeyEqual>::HashMap(Allocator &a)
    : _allocator(&a), _capacity(0), _size(0), _mask(0), _index(NULL),
      _data(NULL), _buffer(NULL) {}
...
#endif // ZEAL_HASH_MAP_H_HEADER_GUARD