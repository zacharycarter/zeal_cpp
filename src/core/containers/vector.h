#ifndef ZEAL_VECTOR_H_HEADER_GUARD
#define ZEAL_VECTOR_H_HEADER_GUARD
...
template <typename T>
inline u32 size(const Vector<T> &v) { return v._size; }
...
template <typename T>
inline void condense(Vector<T> &v) { resize(v, v._size); }
...
{
    ZE_ASSERT(v._size > 0, "The vector is empty");
    v._data[v._size - 1].~T();
    --v._size;
}
...
template <typename T>
inline T *begin(Vector<T> &v) { return v._data; }
...
template <typename T>
inline T *end(Vector<T> &v) { return v._data + v._size; }
...
inline Vector<T>::Vector(Allocator &a)
    : _allocator(&a), _capacity(0), _size(0), _data(NULL) {}

template <typename T>
inline Vector<T>::Vector(const Vector<T> &other)
    : _allocator(other._allocator), _capacity(0), _size(0), _data(NULL)
{
...
{
    ZE_ASSERT(index < _size, "Index out of bounds");
    return _data[index];
}
...
#endif // ZEAL_VECTOR_H_HEADER_GUARD