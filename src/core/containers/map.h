#ifndef ZEAL_MAP_H_HEADER_GUARD
#define ZEAL_MAP_H_HEADER_GUARD
...
template <typename TKey, typename TValue>
bool has(const Map<TKey, TValue> &m, const TKey &key);

/// Returns the value for the given @a key or @a deffault if
/// the key does not exist in the map.
template <typename TKey, typename TValue>
const TValue &get(const Map<TKey, TValue> &m, const TKey &key,
                  const TValue &deffault);

/// Sets the @a value for the @a key in the map.
template <typename TKey, typename TValue>
void set(Map<TKey, TValue> &m, const TKey &key, const TValue &value);

/// Removes the @a key from the map if it exists.
template <typename TKey, typename TValue>
void remove(Map<TKey, TValue> &m, const TKey &key);
...
{
    ZE_ASSERT(n < vector::size(m._data),
              "Index out of bounds (size = %d, n = %d)", vector::size(m._data),
              n);
...
inline s32 dbg_verify(Map<TKey, TValue> &m, u32 n)
{
    ...
    {
        ZE_ASSERT(parent(m, left(m, n)) == n, "Bad RBTree");
        ZE_ASSERT(m._data[left(m, n)].pair.first < m._data[n].pair.first,
                  "Bad RBTree");
    }

    if (right(m, n) != m._sentinel)
    {
        ZE_ASSERT(parent(m, right(m, n)) == n, "Bad RBTree");
        ZE_ASSERT(m._data[n].pair.first < m._data[right(m, n)].pair.first,
                  "Bad RBTree");
    }

    s32 bhL = dbg_verify(m, left(m, n));
    s32 bhR = dbg_verify(m, right(m, n));
    ZE_ASSERT(bhL == bhR, "Bad RBTree");
...
{
    ZE_ASSERT(x < vector::size(m._data),
              "Index out of bounds (size = %d, n = %d)", vector::size(m._data),
              x);
...
u32 size(const Map<TKey, TValue> &m)
{
    ZE_ASSERT(vector::size(m._data) > 0,
              "Bad Map"); // There should be at least sentinel
...
template <typename TKey, typename TValue>
inline const TValue &get(const Map<TKey, TValue> &m, const TKey &key,
                         const TValue &deffault)
{
        ...
        while (x != m._root && color(m, x) == map_internal::BLACK)
        {
            if (x == left(m, parent(m, x)))
            {
                ...
                if (color(m, left(m, y)) == map_internal::BLACK &&
                    color(m, right(m, y)) == map_internal::BLACK)
                {
            ...
            else
            {
                ...
                if (color(m, right(m, y)) == map_internal::BLACK &&
                    color(m, left(m, y)) == map_internal::BLACK)
                {
                    m._data[y].color = map_internal::RED;
                    x = parent(m, x);
                }
...
template <typename TKey, typename TValue>
inline Map<TKey, TValue>::Map(Allocator &a) : _data(a)
{
    map::clear(*this);
}
