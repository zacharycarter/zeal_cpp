...
{
    return shortopt != '\0' && strlen32(arg) > 1 && arg[0] == '-' &&
           arg[1] == shortopt;
}

static bool is_longopt(const char *arg, const char *longopt)
{
    return longopt != NULL && strlen32(arg) > 2 && arg[0] == '-' &&
           arg[1] == '-' && strcmp(&arg[2], longopt) == 0;
}

static int find_option(int argc, const char **argv, const char *longopt,
                       char shortopt)
{
...
const char *CommandLine::get_parameter(int i, const char *longopt,
                                       char shortopt)
{
