...
namespace zeal
{
ConsoleServer::ConsoleServer(Allocator &a) : _clients(a), _commands(a) {}
...
void ConsoleServer::update()
{
    ...
    for (u32 i = 0; i < array::size(_clients); ++i)
    {
        for (;;)
        {
            ...
            cmd.function = NULL;
            cmd.user_data = NULL;
            cmd = hash_map::get(_commands, sjson::parse_string_id(obj["type"]), cmd);
...
void ConsoleServer::register_command(const char *type, CommandFunction function,
                                     void *user_data)
{
    ZE_ENSURE(NULL != type);
    ZE_ENSURE(NULL != function);
...
{
    _console_server =
        ZE_NEW(default_allocator(), ConsoleServer)(default_allocator());
}

void shutdown()
{
    _console_server->shutdown();
    ZE_DELETE(default_allocator(), _console_server);
