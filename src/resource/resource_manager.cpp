...
ResourceManager::ResourceManager(ResourceLoader &rl)
    : _resource_heap(default_allocator(), "resource"), _loader(&rl),
      _type_data(default_allocator()), _rm(default_allocator()),
      _autoload(false) {}
...
const void *ResourceManager::get(StringId64 type, StringId64 name)
{
    ...
    ZE_ASSERT(can_get(type, name), "Resource not loaded #ID(%s)", path.c_str());
...
void ResourceManager::enable_autoload(bool enable) { _autoload = enable; }
...
void ResourceManager::complete_request(StringId64 type, StringId64 name,
                                       void *data)
{
    ResourceEntry entry;
    entry.references = 1;
...
void ResourceManager::register_type(StringId64 type, u32 version,
                                    LoadFunction load, UnloadFunction unload,
...
{
    OnlineFunction func =
        sort_map::get(_type_data, type, ResourceTypeData()).online;
...
{
    OfflineFunction func =
        sort_map::get(_type_data, type, ResourceTypeData()).offline;
...
{
    UnloadFunction func =
        sort_map::get(_type_data, type, ResourceTypeData()).unload;
