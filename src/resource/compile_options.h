#ifndef ZEAL_COMPILE_OPTIONS_H_HEADER_GUARD
#define ZEAL_COMPILE_OPTIONS_H_HEADER_GUARD
...
#define DATA_COMPILER_ASSERT_RESOURCE_EXISTS(type, name, opts)   \
    DATA_COMPILER_ASSERT(opts.resource_exists(type, name), opts, \
                         "Resource not found: '%s.%s'", name, type)

#define DATA_COMPILER_ASSERT_FILE_EXISTS(name, opts)                           \
    DATA_COMPILER_ASSERT(opts.file_exists(name), opts, "File not found: '%s'", \
                         name)
...
struct CompileOptions
{
    ...
    ///
    CompileOptions(DataCompiler &dc, Filesystem &data_filesystem,
...
#endif // ZEAL_COMPILE_OPTIONS_H_HEADER_GUARD