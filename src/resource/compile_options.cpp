...
namespace zeal
{
CompileOptions::CompileOptions(DataCompiler &dc, Filesystem &data_filesystem,
                               DynamicString &source_path, Buffer &output,
                               const char *platform)
    : _data_compiler(dc), _data_filesystem(data_filesystem),
      _source_path(source_path), _output(output), _platform(platform),
...
const char *CompileOptions::source_path() { return _source_path.c_str(); }
...
void CompileOptions::write_temporary(const char *path, const char *data,
                                     u32 size)
{
...
///
Buffer CompileOptions::read() { return read(_source_path.c_str()); }
...
void CompileOptions::get_temporary_path(const char *suffix,
                                        DynamicString &abs)
{
    TempAllocator1024 ta;
    ...
    _data_filesystem.get_absolute_path(ZEAL_TEMP_DIRECTORY, str);
...
const char *CompileOptions::platform() const { return _platform; }
...
int CompileOptions::run_external_compiler(const char *const *argv,
                                          StringStream &output)
{
